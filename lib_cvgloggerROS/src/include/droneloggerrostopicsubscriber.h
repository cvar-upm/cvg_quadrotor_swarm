...
private:
    ...
    std::string tag_name;
public:
    DroneLoggerROSTopicSubscriber(const ros::NodeHandle &n, std::string tag_name_in, const std::string topic_name, std::string (*p_rosmsg2string_f_in)(const T &msg), ros::Time init_rostime_in, int stringStacker_Channel = 0);
    ~DroneLoggerROSTopicSubscriber();
};

template<class T>
DroneLoggerROSTopicSubscriber<T>::DroneLoggerROSTopicSubscriber(const ros::NodeHandle &nIn, std::string tag_name_in, const std::string topic_name, std::string (*p_rosmsg2string_f_in)(const T &msg), ros::Time init_rostime_in, int stringStacker_Channel) :
    debug_string_stacker(stringStacker_Channel)
{
    ...
    topic_subscriber = n.subscribe( topic_name, 5, &DroneLoggerROSTopicSubscriber<T>::topicSubsCallback, this);
}
...
private:
    ...
    std::string tag_name;
public:
    DroneLoggerROSTopicSubscriber_MsgWithHeader(const ros::NodeHandle &n, std::string tag_name_in, const std::string topic_name, std::string (*p_rosmsg2string_f_in)(const T &msg), ros::Time init_rostime_in, int stringStacker_Channel = 0);
    ~DroneLoggerROSTopicSubscriber_MsgWithHeader();
};

template<class T>
DroneLoggerROSTopicSubscriber_MsgWithHeader<T>::DroneLoggerROSTopicSubscriber_MsgWithHeader(const ros::NodeHandle &nIn, std::string tag_name_in, const std::string topic_name, std::string (*p_rosmsg2string_f_in)(const T &msg), ros::Time init_rostime_in, int stringStacker_Channel) :
    debug_string_stacker(stringStacker_Channel)
{
    init_rostime = init_rostime_in;
    ros::NodeHandle n = nIn;
    tag_name = tag_name_in;
    p_rosmsg2string_f = p_rosmsg2string_f_in;
    topic_subscriber = n.subscribe( topic_name, 5, &DroneLoggerROSTopicSubscriber_MsgWithHeader<T>::topicSubsCallback, this);
}
