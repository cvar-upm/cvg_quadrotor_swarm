...
void LowPassFilter::enableSaturation(bool enable, double min, double max) {
...
void LowPassFilter::enableMaxSpeedSaturation(double max_speed) {
	this->max_speed = max_speed; // <0 means disabled
}

double LowPassFilter::getOutput(bool use_last_input) {
	...
	// calculate output
    double alpha = exp(-3*elapsed/tr);
	y_km1 = y_k;
	y_k = alpha*y_km1 + (1-alpha)*u_k;

    double current_speed = (y_k - y_km1)/elapsed;
